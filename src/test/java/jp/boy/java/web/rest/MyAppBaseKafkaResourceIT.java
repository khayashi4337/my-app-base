...
import jp.boy.java.config.KafkaProperties;
...
class MyAppBaseKafkaResourceIT {
    ...
    void setup() {
        ...
        MyAppBaseKafkaResource kafkaResource = new MyAppBaseKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/my-app-base-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/my-app-base-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
